...
{    
	...
	pIOCPModel->_ShowMessage(TEXT("工作者线程启动，ID: %d."), nThreadNo);
...
bool CIOCPModel::Start()
{
	...
	{
		this->_ShowMessage(TEXT("Listen Socket初始化完毕."));
	}
...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	{
		this->_ShowMessage(TEXT("初始化Socket失败，错误代码: %d.\n"), WSAGetLastError());
		return false;
	}
	else
	{
	...
	{  
		this->_ShowMessage(TEXT("绑定 Listen Socket至完成端口失败！错误代码: %d/n"), WSAGetLastError());
	...
	stds::tool t;
	std::wstring s = m_strIP.GetString();
	std::string sStrIP = t.ws2s(s);
	ServerAddress.sin_addr.s_addr = inet_addr(sStrIP.c_str());
	ServerAddress.sin_port = htons(m_nPort);                          

	// 绑定地址和端口
	if (SOCKET_ERROR == bind(m_pListenContext->m_Socket, (struct sockaddr *) &ServerAddress, sizeof(ServerAddress))) 
	{
		this->_ShowMessage(TEXT("bind()函数执行错误.\n"));
		return false;
	}
	else
	{
	...
	{
		this->_ShowMessage(TEXT("Listen()函数执行出现错误.\n"));
		return false;
	}
	else
	{
	...
	if(SOCKET_ERROR == WSAIoctl(
		...
		NULL))  
	{  
		this->_ShowMessage(TEXT("WSAIoctl 未能获取AcceptEx函数指针。错误代码: %d\n"), WSAGetLastError());
		this->_DeInitialize();
		return false;  
	}  
	...
	if(SOCKET_ERROR == WSAIoctl(
		...
		NULL))  
	{  
		this->_ShowMessage(TEXT("WSAIoctl 未能获取GuidGetAcceptExSockAddrs函数指针。错误代码: %d\n"), WSAGetLastError());
		this->_DeInitialize();
		return false; 
	}  
...
bool CIOCPModel::_PostAccept( PER_IO_CONTEXT* pAcceptIoContext )
{
	...
	{  
		...
		{  
			_ShowMessage(TEXT("投递 AcceptEx 请求失败，错误代码: %d"), WSAGetLastError());
			return false;  
		}  
	} 
...
bool CIOCPModel::_DoAccpet( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	CString temp,temp2;
	temp = inet_ntoa(ClientAddr->sin_addr);
	temp2 = pIoContext->m_wsaBuf.buf;
	this->_ShowMessage( _T("客户端 %s:%d 连入."), temp, ntohs(ClientAddr->sin_port) );
	this->_ShowMessage( _T("客户额 %s:%d 信息：%s."),temp, ntohs(ClientAddr->sin_port),temp2 );
...
bool CIOCPModel::_PostRecv( PER_IO_CONTEXT* pIoContext )
{
	...
	{
		this->_ShowMessage(TEXT("投递第一个WSARecv失败！"));
		return false;
	}
	return true;
}
...
bool CIOCPModel::_AssociateWithIOCP( PER_SOCKET_CONTEXT *pContext )
{
	...
	if (NULL == hTemp)
	{
		this->_ShowMessage(__T("执行CreateIoCompletionPort()出现错误.错误代码：%d"),GetLastError());
		return false;
	}
